  ...
  io.sockets.on('connection', function (socket) {
  	...
  	socket.on('createRoom', function (roomName) {
  		if (rooms.indexOf(roomName) == -1) {
  			...
  			roomList[socket.room] += new Map();
  			roomList[socket.room].set(socket.username, socket.scores)
			  
  			io.sockets.in(socket.room).emit('allUsers', roomList[socket.room]);
			io.sockets.in(socket.room).emit('startScores', roomList[socket.room]);
  		} else {
  			socket.emit('roomCreatedError', roomName + ' has already been chosen')
  		}
  	})
  	...
  	socket.on('joinRoom', function (roomName) {
  		...
  		io.sockets.in(socket.room).emit('allUsers', roomList[socket.room].keys());
  		io.sockets.in(socket.room).emit('startScores', roomList[socket.room]);
